...
namespace Microsoft.Unity.VisualStudio.Editor {
	internal class VisualStudioKiroInstallation : VisualStudioInstallation {
		...
		public static IEnumerable<IVisualStudioInstallation> GetVisualStudioInstallations() {
			...
			// Search all drives for Program Files\Kiro
			try {
				var drives = System.IO.DriveInfo.GetDrives();
				foreach (var drive in drives) {
					if (drive.IsReady && drive.DriveType == System.IO.DriveType.Fixed) {
						// Check D:\Program Files\Kiro, E:\Program Files\Kiro, etc.
						candidates.Add(IOPath.Combine(drive.Name, "Program Files", "Kiro", "Kiro.exe"));
						candidates.Add(IOPath.Combine(drive.Name, "Program Files (x86)", "Kiro", "Kiro.exe"));
					}
				}
			} catch {
